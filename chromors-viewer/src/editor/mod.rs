...
mod tests {
    ...
    fn test_m1_skeleton_evaluate() {
        ...
        let edge1 = g.connect(
            PortAddr { node: load, side: Side::Out, index: 0 },
            PortAddr { node: exposure, side: Side::In, index: 0 },
        ).unwrap();
        
        let edge2 = g.connect(
            PortAddr { node: exposure, side: Side::Out, index: 0 },
            PortAddr { node: viewer, side: Side::In, index: 0 },
        ).unwrap();
