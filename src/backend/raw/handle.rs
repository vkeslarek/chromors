...
impl RawSource {
    ...
    pub(crate) unsafe fn open_ptr(&self) -> Result<*mut raw::libraw_data_t, crate::error::Error> {
        use std::ffi::CString;
        let ptr = unsafe { raw::libraw_init(0) };
        if ptr.is_null() {
            return Err(crate::error::Error::Raw("libraw_init failed".into()));
        }
        let rc = match self {
            RawSource::File(path) => {
                let c_path = CString::new(path.as_str())
                    .map_err(|_| crate::error::Error::Raw("path contains null byte".into()))?;
