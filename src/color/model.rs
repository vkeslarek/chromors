...
/// decode transformation from that model to linear sRGB.
///
/// This is a pre-step that runs before the transfer function decode and
/// primary matrix. Formats like CMYK, YCbCr, and Lab aren't RGB — they must
/// first be converted to RGB coordinates before the standard pipeline applies.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[repr(u32)]
pub enum ColorModelTransform {
    /// No transformation — the data is already RGB.
    None = 0,
    /// CMYK with opaque alpha → RGB (inverted, scaled by K).
    CmykToRgb = 1,
    /// CMYK with separate alpha → RGB (inverted, scaled by K).
    CmykAToRgb = 2,
    /// YCbCr (full-range, centered at 0.5) → RGB.
    YCbCrToRgb = 3,
    /// CIE L*a*b* → linear sRGB via D50 XYZ and a combined Bradford+primary matrix.
    LabToRgb = 4,
}

impl ColorModelTransform {
    /// Decode 4 pixels at once using SIMD-width `f32x4` registers.
    ///
    /// Returns `(r, g, b)` — alpha is passed through in `c3` for CMYK scaling
    /// but is not returned (caller handles alpha separately).
    ...
    /// Decode a single pixel (scalar path).
    ///
    /// `ch` is a 4-component slice `[c0, c1, c2, alpha]`. Returns `[r, g, b]`.
...
///
/// Inputs are normalized to the range used by the engine:
/// - `l_norm` in `[0..1]` representing `L* = 100 × l_norm`.
/// - `a_norm` in `[-1..1]` representing `a* = 128 × a_norm`.
/// - `b_norm` in `[-1..1]` representing `b* = 128 × b_norm`.
///
/// The conversion path is: Lab → D50 XYZ → (Bradford D50→D65) → linear sRGB.
/// The Bradford adaptation and D65→sRGB primary matrix are combined into a
/// single baked matrix for performance.
///
/// Output may be outside `[0..1]` for wide-gamut Lab colors. No clamping is
/// applied — the downstream primary matrix (sRGB→dst) handles out-of-gamut
/// values, making this equivalent to Lab→XYZ→dst without lossy intermediate
/// clamping.
