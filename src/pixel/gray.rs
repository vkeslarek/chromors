...
impl Pixel for Gray<f32> {
    ...
    fn pack_one(rgba: [f32; 4], _mode: AlphaPolicy) -> Self {
        Self {
            v: rgba[0] * 0.2126 + rgba[1] * 0.7152 + rgba[2] * 0.0722,
        }
    }
    fn pack_x4(rr: f32x4, gg: f32x4, bb: f32x4, _aa: f32x4, _mode: AlphaPolicy, out: &mut [Self]) {
        let r = rr.to_array();
        ...
        for i in 0..4 {
            out[i] = Self {
                v: r[i] * 0.2126 + g[i] * 0.7152 + b[i] * 0.0722,
            };
        }
    }
}
...
impl Pixel for Gray<f16> {
    ...
    fn pack_one(rgba: [f32; 4], _mode: AlphaPolicy) -> Self {
        ...
        Self {
            v: f16::from_f32(v.clamp(0.0, 65504.0)),
        }
    }
    fn pack_x4(rr: f32x4, gg: f32x4, bb: f32x4, _aa: f32x4, _mode: AlphaPolicy, out: &mut [Self]) {
        let r = rr.to_array();
...
impl Pixel for GrayAlpha<u8> {
    ...
    fn pack_one(rgba: [f32; 4], _mode: AlphaPolicy) -> Self {
        Self {
            v: ((rgba[0] * 0.2126 + rgba[1] * 0.7152 + rgba[2] * 0.0722) * 255.0 + 0.5) as u8,
            a: (rgba[3].clamp(0.0, 1.0) * 255.0 + 0.5) as u8,
        }
    }
    fn pack_x4(rr: f32x4, gg: f32x4, bb: f32x4, aa: f32x4, _mode: AlphaPolicy, out: &mut [Self]) {
        ...
        for i in 0..4 {
            let v = r[i] * 0.2126 + g[i] * 0.7152 + b[i] * 0.0722;
            out[i] = Self {
                v: (v.clamp(0.0, 1.0) * 255.0 + 0.5) as u8,
                a: (a[i].clamp(0.0, 1.0) * 255.0 + 0.5) as u8,
            };
        }
    }
}
...
impl Pixel for GrayAlpha<u16> {
    ...
    fn pack_one(rgba: [f32; 4], _mode: AlphaPolicy) -> Self {
        Self {
            v: ((rgba[0] * 0.2126 + rgba[1] * 0.7152 + rgba[2] * 0.0722).clamp(0.0, 1.0) * 65535.0
                + 0.5) as u16,
    ...
    fn pack_x4(rr: f32x4, gg: f32x4, bb: f32x4, aa: f32x4, _mode: AlphaPolicy, out: &mut [Self]) {
        ...
        for i in 0..4 {
            ...
            out[i] = Self {
                v: (v * 65535.0 + 0.5) as u16,
                a: (a[i].clamp(0.0, 1.0) * 65535.0 + 0.5) as u16,
            };
        }
    }
}
