...
fn main() {
    ...
    if env::var("CARGO_FEATURE_SAM2").is_ok() { cmd.arg("sam2"); has_features = true; }
    if env::var("CARGO_FEATURE_SAM3").is_ok() { cmd.arg("sam3"); has_features = true; }
    if env::var("CARGO_FEATURE_CASCADEPSP").is_ok() { cmd.arg("cascadepsp"); has_features = true; }
