...
pub struct SwinIrConfig {
    ...
    /// If true, input is normalized to [0, 1]; otherwise [-1, 1] (default: true).
    pub normalize_0_1: bool,

    /// Clamp output values to [0, 1] before converting to u8 (default: true).
...
impl SwinIrModel {
    ...
    ///
    /// Output size depends on model variant: 1× for denoising, 2×/4× for super-resolution.
    pub fn restore<B: AiBackend>(
        &mut self,
        image: &Image2D<B>,
        ...
        let (shape, slice) = outputs[out_key]
            ...
            .map_err(|e| chromors::error::Error::Backend(format!("SwinIR output: {e:?}")))?;

        let out_h = shape[2] as usize;
        let out_w = shape[3] as usize;

        // Infer upscale factor; clamp to 1 for denoising models where pad_h == out_h.
        let scale = (out_h / pad_h).max(1);
        let crop_h = h * scale;
        let crop_w = w * scale;
