...
impl Default for CascadePspConfig {
    fn default() -> Self {
        Self { global_size: 512, pad_multiple: 8, local_window: 512, skip_uniform_patches: true }
    }
}
...
impl CascadePspModel {
    ...
    pub fn config(&self) -> &CascadePspConfig { &self.config }
    ...
    pub fn refine<B: AiBackend>(
        ...
        let mask_small = mask
            .resize(sw as f64 / mask.width() as f64, None, Some(sh as f64 / mask.height() as f64), None)
            .convert(RGB_U8_LAYOUT);

        let img_bytes = img_small.pull(&RamImageTarget, Region::full((sw as i32, sh as i32), Lod(0)))?;
        let mask_bytes = mask_small.pull(&RamImageTarget, Region::full((sw as i32, sh as i32), Lod(0)))?;

        let image_arr = hwc_u8_to_nchw_normalized(&img_bytes, sw, sh);
        let mask_arr = hwc_u8_to_mask_nchw(&mask_bytes, sw, sh);

        let out_arr = self.forward_padded(image_arr, mask_arr)
    ...
    fn refine_local_internal(&mut self, image: Array4<f32>, mask: Array4<f32>) -> ort::Result<Array4<f32>> {
        use ndarray::s;
        ...
        for y_idx in 0..=(h / step_size) {
            for x_idx in 0..=(w / step_size) {
                ...
                if !used_starts.insert(sy * w + sx) { continue; }
        ...
        for i in 0..(b * h * w) {
            ...
            out_s[i] = if wt_s[i] == 0.0 { seg_norm } else { comb_s[i] / wt_s[i] };
        }
        Ok(out)
    }
}
...
fn hwc_u8_to_nchw_normalized(bytes: &[u8], w: usize, h: usize) -> Array4<f32> {
    ...
    for y in 0..h {
        for x in 0..w {
            let idx = (y * w + x) * 3;
            for c in 0..3 {
                arr[[0, c, y, x]] = (bytes[idx + c] as f32 / 255.0 - IMAGENET_MEAN[c]) / IMAGENET_STD[c];
            }
        }
    }
    arr
...
fn hwc_u8_to_mask_nchw(bytes: &[u8], w: usize, h: usize) -> Array4<f32> {
    ...
    for y in 0..h {
        for x in 0..w {
            let bin = if bytes[(y * w + x) * 3] > 127 { 1.0f32 } else { 0.0 };
