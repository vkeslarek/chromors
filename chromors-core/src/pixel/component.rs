...
//!
//! The [`Component`] trait abstracts over the scalar types used for individual
//! pixel channels: `u8`, `u16`, `f16` (`half`), and `f32`. It provides uniform
//! conversion to/from normalized `f32` with appropriate scaling.
...
pub trait Component: Copy + 'static + Pod + Zeroable {
    /// The zero value for this component type.
    const ZERO: Self;

    /// The "one" value: 255 for u8, 65535 for u16, 1.0 for floats.
    const ONE: Self;

    /// The maximum value in normalized `f32` space (255.0 for u8, 65535.0 for
    /// u16, 1.0 for floats).
    const MAX_ONE_F32: f32;

    /// Converts the component to normalized `f32` in range `[0, 1]` for
    /// unsigned integers, or returns the direct value for floats.
    fn to_f32(self) -> f32;

    /// Converts a normalized `f32` value to the component, clamping to the
    /// valid range (`[0, 1]`).
...
impl Component for half::f16 {
    ...
    const ONE: half::f16 = half::f16::from_bits(0x3C00); // 1.0 in half
...
mod tests {
    ...
    fn u8_conversion() {
        ...
        assert_eq!(u8::from_f32_clamped(0.5), 128);
    ...
    fn u16_conversion() {
        ...
        assert_eq!(u16::from_f32_clamped(0.5), 32768);
    }
