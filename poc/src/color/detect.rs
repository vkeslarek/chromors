...
//!
//! Used by PNG and TIFF readers to identify the color space from embedded
//! chromaticity metadata and ICC profiles. Pure functions, no I/O.

use crate::color::primaries::{RgbPrimaries, WhitePoint};
use crate::color::space::ColorSpace;

/// Match `(wx, wy, rx, ry, gx, gy, bx, by)` chromaticity values against
/// known primaries+whitepoint combos.
///
/// Returns `Some((RgbPrimaries, WhitePoint))` if the values match within `tol`.
/// Recommended tolerance: 0.002 for TIFF, 0.001 for PNG.
...
///
/// Contains the identified [`ColorSpace`] (or `None` if the profile wasn't
/// recognized) and a copy of the raw ICC profile bytes for fallback use.
pub struct IccClassification {
    /// The recognized color space, or `None` if the profile wasn't matched.
    pub color_space: Option<ColorSpace>,
    /// A copy of the raw ICC profile bytes.
    pub raw: Vec<u8>,
}

impl IccClassification {
    /// Classify an ICC profile by parsing the header and matching the
    /// description tag against known profile names.
    ///
    /// Returns a known [`ColorSpace`] only when all of these hold:
    /// - `profile_class == "mntr"` (display device profile)
    /// - `color_space == "RGB "` (four-byte signature)
    /// - The profile description string matches a known entry (sRGB, Adobe RGB,
    ///   Display P3, DCI P3, ProPhoto, or Rec.2020)
    ...
    /// Normalise a profile description string for matching: lowercases,
    /// replaces non-alphanumeric characters with a single space, trims.
    ...
    ///
    /// Parses the tag table starting at offset 132, finds the `'desc'` tag,
    /// and reads its null-terminated string content.
