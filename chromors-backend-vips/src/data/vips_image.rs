...
impl FileImageSource {
    pub fn new(filename: &str) -> Result<Self, Error> {
        ensure_init();
        let c = std::ffi::CString::new(filename).map_err(|_| Error::Vips("invalid filename".into()))?;
        let ptr = unsafe { ffi::vips_image_new_from_file(c.as_ptr(), std::ptr::null_mut::<std::ffi::c_void>()) };
        if ptr.is_null() { return Err(Error::Vips(vips_error())); }
        ...
        let color_space = if matches!(model, ColorModel::Rgb | ColorModel::ScRgb) {
            ...
            let mut len: usize = 0;
            let has_icc = unsafe { ffi::vips_image_get_blob(ptr, icc_name.as_ptr(), &mut data, &mut len) == 0 };
            if has_icc && !data.is_null() && len > 0 {
                let bytes = unsafe { std::slice::from_raw_parts(data as *const u8, len) };
                IccClassification::classify_icc_profile(bytes).color_space.unwrap_or(default_cs)
            } else { default_cs }
        } else { default_cs };
        unsafe { ffi::g_object_unref(ptr as *mut std::ffi::c_void) };
        let spec = Arc::new(ImageKind::new(PixelLayout { storage, model, alpha, color_space }, width, height));
...
impl Source<VipsBackend> for FileImageSource {
    type Kind = ImageKind;
    fn spec(&self) -> Arc<ImageKind> { self.spec.clone() }
    ...
    fn dyn_hash(&self, state: &mut dyn Hasher) { state.write(self.filename.as_bytes()); }
}
...
impl Source<VipsBackend> for RamImageSource {
    ...
    fn fetch(&self, _ctx: &(), _wu: &Region) -> Result<Buffer<VipsBackend>, Error> {
        ...
        let ptr = unsafe { ffi::vips_image_new_from_memory_copy(self.data.as_ptr() as *const std::ffi::c_void, self.data.len(), self.spec.width, self.spec.height, bands, vips_format) };
        if ptr.is_null() { return Err(Error::Vips(vips_error())); }
        Ok(Buffer { payload: Arc::new(VipsHandle { ptr }), spec: self.spec.clone() })
    }
    fn lower(&self, cx: &mut VipsBuilder) {
    ...
    fn dyn_hash(&self, state: &mut dyn Hasher) {
        ...
        if self.data.len() >= 16 { state.write(&self.data[..16]); }
    }
}
...
impl Target<ImageKind, VipsBackend> for RamImageTarget {
    type Out = Vec<u8>;
    fn extract(&self, buf: &Buffer<VipsBackend>, _wu: &Region, _ctx: &()) -> Result<Self::Out, Error> {
        let mut size: usize = 0;
        let ptr = unsafe { ffi::vips_image_write_to_memory(buf.payload.ptr, &mut size as *mut usize) };
        if ptr.is_null() { return Err(Error::Vips(vips_error())); }
...
pub trait VipsImageExt {
    ...
    fn from_bytes(data: Vec<u8>, width: i32, height: i32, layout: PixelLayout) -> Image2D<VipsBackend>;
}
...
impl VipsImageExt for Image2D<VipsBackend> {
    ...
    fn from_bytes(data: Vec<u8>, width: i32, height: i32, layout: PixelLayout) -> Image2D<VipsBackend> {
        ...
        let src = RamImageSource { spec: spec.clone(), data };
