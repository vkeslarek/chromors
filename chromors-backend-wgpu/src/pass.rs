...
pub struct StagingSource {
    spec: Arc<chromors_core::ImageKind>,
    buffer: Arc<GpuBuffer>,
}

impl Source<GpuBackend> for StagingSource {
    type Kind = chromors_core::ImageKind;

    fn spec(&self) -> Arc<chromors_core::ImageKind> {
        self.spec.clone()
    }
    ...
    let stubs: Vec<StubInput> = rebuilt_children
        ...
        .map(|(child, orig_input)| StubInput {
            kind: orig_input
                .spec()
                .as_any()
                .downcast_ref::<chromors_core::ImageKind>()
        ...
        cuts.par_iter()
            .map(|cut_node| {
                ...
                let source_node: Arc<Node<GpuBackend>> = Arc::new(Node::from_source(
                    source as Arc<dyn crate::AnySource<GpuBackend>>,
                ));
