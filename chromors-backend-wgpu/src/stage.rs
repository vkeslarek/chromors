    ...
    K: GpuView,
{
    ...
    fn fetch(&self, _ctx: &crate::GpuContext, wu: &K::WorkUnit) -> Result<Buffer<GpuBackend>, Error> {
        self.serve(wu)
    }
    ...
    fn lower(&self, cx: &mut GpuBuilder) {
        ...
        let Some(typed) = K::WorkUnit::typed(&wu) else {
            cx.fail(Error::InvalidWorkUnit("boundary source: work unit shape mismatch".into()));
            return;
        };
